...
class SemanticNetwork:
    def __init__(self, data, variables, key) -> None:
        self.data = data
    ...
    def coocurrence_multiple(self):
        ...
        for var in self.variables:
            df_var = self.data[[self.key, var]]
            df_var = df_var.drop_duplicates()
            df_var = (
                df_var.groupby([self.key, var])[self.key]
                .count()
                .rename("weight")
                .reset_index()
            )
        ...
        df_co = pd.merge(self.fin, self.fin, on=self.key)
    ...
    def get_top_nodes(self, top_n=300):
        ...
        nodes_attr = self.fin[[self.key, "data", "entity"]]
        nodes_attr = (
            nodes_attr.groupby(["data", "entity"])[self.key]
            .count()
            .rename("size")
            .reset_index()
        )
